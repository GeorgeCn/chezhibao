...
/*
 * Copied from EasyCorp\Bundle\EasyDeployBundle\Deployer\DefaultDeployer
 * 为了修改一些行为
 */
...
abstract class Deployer extends AbstractDeployer
{
    ...
    final public function deploy() : void
    {
        ...
        $this->log('<h3>Restart supervisord</>');
    ...
    private function doSetPermissions() : void
    {
        ...
        if ('chmod' === $permissionMethod) {
            $this->runRemote(sprintf('sudo chmod -R %s %s', $this->getConfig(Option::permissionMode), $writableDirs));
    ...
    private function restartSupervisord() : void
    {
        $this->runLocal(sprintf('sudo supervisorctl restart all'));
    }
    ...
    private function deleteOldReleases(Server $server, array $releaseDirs) : void
    {
        ...
        $absoluteDirsToRemove = array_map(function ($v) use ($server) {
            $path = sprintf('%s/releases/%s', $this->getConfig(Option::deployDir), $v);
            $this->runOnServer(sprintf('sudo chmod -R %s %s', $this->getConfig(Option::permissionMode), $path), $server);
            return $path;
    ...
    {
        $this->log('<h3>Generate version.yml</>');
        $this->runRemote(sprintf('{{ console_bin }} app:version:update %s --no-debug --env=%s', $this->revision, $this->getConfig(Option::symfonyEnvironment)));
    ...
    {
        $this->log('<h3>DoSetPermissions again</>');
        $this->doSetPermissions();
    }
}
