 ...
 * HPL Customization Date 2016-11-23
 */
...
(function (factory) {
  ...
  Viewer.prototype = {
    ...
    build: function () {
      ...
      this.$sidebar = $sidebar = $viewer.find('.viewer-sidebar');
      this.$titleList = $sidebar.find('.viewer-title-list');
      ...
      $sidebar.addClass(!options.sidebar ? CLASS_HIDE : getResponsiveClass(options.sidebar));
    ...
    initContainer: function () {
      this.container = {
        width: this.$viewer.width(),//$window.innerWidth()
        height: this.$viewer.height() //$window.innerHeight()
      };
    },
    ...
    initList: function () {
      ...
      $titleList.html(titleList.join(''))

      this.$items = $list.children();
      this.$titleItems = $titleList.children();
    ...
    wheel: function (event) {
      ...
      if($(event.target)==this.$sidebar || this.$sidebar.find(event.target).length){
    ...
    view: function (index) {
      ...
      $titleItem = this.$titleItems.eq(index);
      ...
      //if (this.isViewed) {
        ...
        this.$titleItems.eq(this.index).removeClass(CLASS_ACTIVE);
      //}

      $item.addClass(CLASS_ACTIVE);
      $titleItem.addClass(CLASS_ACTIVE);
          ...
          if (this.length) {
            if (this.isViewed) {
              ...
              if (index >= 0) {
                ...
                this.$titleItems.eq(this.index).addClass(CLASS_ACTIVE);
              }
            }
          } else {
            this.$image = null;
